...
export function DunGen(templates: TemplateRoom[], opts?: DunGenOpts) {
    ...
    let dungeon = new Dungeon(opts.width!, opts.height!);
    ...
    dungeon = dungeon.map( (tile, x, y, dungeon) => {
        ...
        if (x > 0 && x < dungeon.width - 1 && y > 0 && y < dungeon.height - 1) {
            ...
            if (dungeon.tiles.get(x + xy.x, y + xy.y) === oppositeDirection(tile)) {
                return tile === Direction.BOTTOM ? Direction.BOTTOM : 0;
            } else {
    ...
    for (let i = 0; i < dungeon.width - 1; i ++) {
        for (let j = 0; j < dungeon.width - 1; j ++) {
            if (dungeon.tiles.get(i, j) === 0 && dungeon.tiles.get(i + 1, j) === 0) {
                ...
                } else if (bl === Direction.BOTTOM && br === 1) {
...
export class Dungeon {
    ...
    constructor(public width: number, public height: number, initializer: (x: number, y: number) => number = () => -1) {
    ...
    map(fn: (tile: number, x: number, y: number, dungeon: this) => number): Dungeon {
        return new Dungeon(this.width, this.height, (x, y) => {
...
function fyShuffle<T>(arr: T[]): T[] {
    ...
    for (let i = arr.length - 1; i > 0; i --) {
        let swp = Math.floor(Math.random() * (i + 1));
        let temp = arr[i];
        arr[i] = arr[swp];
        arr[swp] = temp;
    }
